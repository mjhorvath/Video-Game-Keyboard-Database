...
#declare Pyramid3 = intersection
{
//	#local leg_dst = tan(pi/3);
//	#local rot_ang = degrees(acos(1/leg_dst));
//	#local pyr_hgh = sin(acos(1/leg_dst)) * leg_dst;
	#local leg_dst = 1;
	#local rot_ang = atan2d(2,1);
	#local pyr_hgh = 2;
	plane { +y, 0 rotate z * +rot_ang translate -x }
	plane { +y, 0 rotate z * -rot_ang translate +x }
	plane { +y, 0 rotate x * -rot_ang translate -z }
	plane { +y, 0 rotate x * +rot_ang translate +z }
	plane { -y, 0 }
	bounded_by {box {<-1,0,-1>, <1,pyr_hgh,1>}}
	translate x + z
	scale 1/2
}   
...
object
{
	Pyramid3
	scale 1/10
//	scale y// * sin(acos(1/tan(pi/3))) * tan(pi/3)
