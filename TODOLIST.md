...
* Several routines rely very heavily on the database tables not having gaps in 
  them. In my PHP code I need to switch to using "foreach" loops instead of the 
  numerical "for" loops. Or, start using the "array_key_exists" function a lot 
  more often than I have been. Ditto for the JavaScript code. Also, I should 
  start relying on the stored (from the database) index numbers instead of the 
  ones generated by the PHP loops. Ideally, the site should still work properly 
  after re-indexing all the MySQL tables!
