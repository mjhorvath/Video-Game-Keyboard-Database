<?php
	...
	$default_svg_bool	= 0;		// set by selDefaultsAll(), utilized by checkURLParameters()
	$default_ten_bool	= 1;		// set by selDefaultsAll(), utilized by checkURLParameters()
	$default_vert_bool	= 0;		// set by selDefaultsAll(), utilized by checkURLParameters()
	...
	function getURLParameters()
	{
		...
		$vert_bool	= array_key_exists("vrt", $_GET) ? intval(ltrim($_GET["vrt"], "0"))	: null;
	}
...
//			$fix_url = true;	// don't uncomment this!
		...
		if ($vert_bool === null)
		{
			//error_log("vert_bool is null", 0);
			$vert_bool = $default_vert_bool;
			$fix_url = true;
		}

		$php_url = "keyboard-diagram-" . $game_seo . ".php?sty=" . $style_id . "&lay=" . $layout_id . "&fmt=" . $format_id	. "&ten=" . $ten_bool	. "&vrt=" . $vert_bool;
		$svg_url = "keyboard-diagram-" . $game_seo . ".svg?sty=" . $style_id . "&lay=" . $layout_id . "&fmt=" . 1		. "&ten=" . $ten_bool	. "&vrt=" . $vert_bool;
	...
	// cleaning!
	// this function is for "output-submit.php", need to create a version of this function for "output-svg.php" as well
...
"								<div id=\"" . $in_id . "\" class=\"" . $in_class . "\">" . cleantextHTML($in_value) . "</div>\n";
	...
	function checkForErrors()
	{
		...
		// should check that $vert_bool is in the correct range too
		if (!$game_name)
		{
